  ...
  <div>
    ...
    <el-card class="t-box-card">
      ...
      <el-table class="t-table-list" ref="table" border stripe size="small" tooltip-effect="dark"
          ...
          <el-table-column prop="isDefault" label="是否为默认权限" :show-overflow-tooltip="true" align="center">
            <template slot-scope="scope">
              <span>{{scope.row.isDefault ? '是':'否'}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="isPublic" label="是否为公共权限" :show-overflow-tooltip="true" align="center">
            <template slot-scope="scope">
...
<script>
import { getRolesList, deleteRole } from '@/server/system-module'
import addEdit from './role-add-edit'
...
export default {
  ...
  methods: {
    initDataList (SkipCount = 0, pageSize = this.pageSize) {
      ...
      getRolesList(data).then((dataList) => {
